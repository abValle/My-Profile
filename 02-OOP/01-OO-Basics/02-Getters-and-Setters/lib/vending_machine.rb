class VendingMachine
  ...
  def buy_snack
    ...
    if @user_balance_cents / snack_price_cents >= 1 && @user_balance_cents / snack_price_cents <= @snack_count
      @snack_count -= @user_balance_cents / snack_price_cents
      @user_balance_cents -= (@user_balance_cents / snack_price_cents) * snack_price_cents
    elsif @user_balance_cents / snack_price_cents > @snack_count
      @user_balance_cents -= @snack_price_cents * @snack_count
      @snack_count = 0
    else
      puts "no money"
    end
